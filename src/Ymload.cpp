...
unsigned char	*CYmMusic::depackFile( intptr_t checkOriginalSize)
{
  lzhHeader_t *pHeader;
  ymu8	*pNew;
  ymu8	*pSrc;
  ...
  intptr_t	packedSize = ( intptr_t )ReadLittleEndian32((ymu8*)&pHeader->packed);

  checkOriginalSize -= ( intptr_t )(pSrc) - ( intptr_t )(pBigMalloc);
  ...
  if (packedSize <= checkOriginalSize)
    {
      ...
      // Arda : I have no idea but I had to comment this out
      //      delete pDepacker;
...
ymbool	CYmMusic::load(const char *fileName)
{
  ...
  //---------------------------------------------------
  fileSize = (intptr_t) fileSizeGet(in);
